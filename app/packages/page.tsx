...
export default function PackagesPage() {
  ...
  return (
    <main className="min-h-screen bg-stone-50 overflow-x-hidden pt-20">
      ...
      {/* Editorial Header */}
      <section className="bg-primary text-primary-foreground py-24 pb-32 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-4xl">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-secondary/80 border border-primary-foreground/10 text-accent text-sm font-medium tracking-widest uppercase mb-6 shadow-xl">
                Absolute Transparency
              </div>
              <h1 className="text-5xl sm:text-6xl md:text-7xl font-display font-bold text-white mb-6 tracking-tight leading-[1.05]">
                Productized <br />
      ...
      {/* Comparison Matrix */}
      <section className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 -mt-16 mb-24">
            ...
            >
              ...
              <div className="p-8 flex-grow">
                <div className="space-y-10">
                  {categories.map((cat) => (
                    <div key={cat}>
                      <h4 className="text-xs font-bold uppercase tracking-widest text-stone-400 mb-4 pb-2 border-b border-stone-100">
                        {cat}
                      </h4>
                      <ul className="space-y-4">
                        {pkg.specs[cat].map((spec, specIdx) => (
                          <li
                            key={specIdx}
                            className="flex flex-col sm:flex-row sm:items-start gap-3 text-sm text-stone-700 leading-relaxed"
                          >
                            <Check className="w-4 h-4 text-accent shrink-0 mt-0.5 hidden sm:block" />
