    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          >
            {/* Drawer Header */}
            <div className="sticky top-0 z-10 bg-primary/95 backdrop-blur border-b border-primary-foreground/10 p-6 flex items-center justify-between">
              <div>
              ...
              <button
                onClick={onClose}
                className="w-10 h-10 bg-secondary/50 text-primary-foreground/70 hover:text-white hover:bg-secondary rounded-full flex items-center justify-center transition-all"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-12">
              {packageData.map((pkg) => (
                <div key={pkg.grade} className="bg-secondary/20 border border-primary-foreground/10 p-6">
                  <div className="flex items-center justify-between border-b border-primary-foreground/10 pb-4 mb-6">
                    <h4 className="text-xl font-display font-bold text-primary-foreground">{pkg.grade} Finish</h4>
                    <div className="text-xs font-bold uppercase tracking-widest text-accent bg-accent/10 px-3 py-1">
